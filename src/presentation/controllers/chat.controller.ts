...
export class ChatController {
  ...
  public getSessions = async (req: Request, res: Response): Promise<void> => {
    ...
    const sessions = await this._getSessionsUC.execute(userId);
  ...
  public createSession = async (req: Request, res: Response): Promise<void> => {
    const userId = (req as any).user.id;
    const session = await this._createSessionUC.execute(userId);
  ...
  public deleteSession = async (req: Request, res: Response): Promise<void> => {
    ...
    await this._deleteSessionUC.execute({ userId, sessionId });
    res.status(HttpStatus.OK).json(ResponseModel.success("Session deleted", null));
  };

  public getSessionMessages = async (req: Request, res: Response): Promise<void> => {
    const userId = (req as any).user.id;
    const { sessionId } = req.params;
    const messages = await this._getSessionMessagesUC.execute({ userId, sessionId });
